...
async function main() {
	...
	console.log(
		"Think of something. Answer with: yes / no / sometimes / unknown.\n"
	)
	...
	while (true) {
		...
		if (reply.type === "guess") {
			const ok = (await ask(`Guess: ${reply.text} — correct? (y/n) `))
				.trim()
				.toLowerCase()
			if (ok.startsWith("y")) {
				console.log(`\nWon in ${asked} questions.`)
				break
			}
			asked++
			if (asked >= MAX_Q) {
				console.log("\nOut of budget — you win.")
				break
			}
		...
		if (asked >= MAX_Q) {
			...
			const ok = (await ask(`Final guess: ${final.text} — correct? (y/n) `))
				.trim()
				.toLowerCase()
