...
export async function seedSlang(jsonPath = "assets/hogymondom.json") {
	...
	const existing = new Set(
		(db.prepare("SELECT word FROM slang").all() as { word: string }[]).map(
			(r) => r.word
		)
	)
	...
	log.info(
		{ total: entries.length, pending: pending.length },
	...
	for (let i = 0; i < pending.length; i += batchSize) {
		...
		log.info(
			{ done: Math.min(i + batchSize, pending.length), of: pending.length },
			"slang: seeded batch"
		)
	}
}
...
export async function lookupSlang(
	...
	if (!cache) {
		...
		log.info({ entries: cache.length }, "slang: dictionary loaded")
	}
