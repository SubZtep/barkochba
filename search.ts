...
function styleForModel(model: string | undefined): {
  class?: string
  wrap: (id: string, label: string) => string
} {
  if (!model)
    return {
      wrap: (id, label) => `${id}["${label}"]`
    }
  if (MODEL_INDEX[model] === undefined)
    MODEL_INDEX[model] = Object.keys(MODEL_INDEX).length
  const idx = MODEL_INDEX[model]!
  return {
    class: `model${idx}`,
    wrap: MODEL_SHAPES[idx % MODEL_SHAPES.length]!
  }
}
...
function renderMermaid(nodes: TraceNode[]): string {
  ...
  for (const n of canonical.values()) {
    const wrapMax =
      n.kind === "tool_result" || n.kind === "final_answer" ? 220 : 100
    const label0 = wrapLabel(n.label, wrapMax)
    const label =
      n.durationMs !== undefined
        ? `${label0}<br/><i>${(n.durationMs / 1000).toFixed(1)}s</i>`
        : label0
    if (label) {
      let shape: string
  ...
  for (const call of message.tool_calls) {
    if (call.type !== "function") {
      log.warn(
        ...
        "Unknown tool call type, skipping"
      )
      continue
    }
