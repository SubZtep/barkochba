...
async function callGuesser(
	messages: OpenAI.ChatCompletionMessageParam[]
): Promise<GuesserReply> {
...
async function main() {
	...
	while (true) {
		...
		if (reply.type === "guess") {
			answererMessages.push({
				role: "user",
				content: `Is the answer "${reply.text}"?`
			})
			const confirmation = await callAnswerer(answererMessages, answSys)
			answererMessages.push({
				role: "assistant",
				content: JSON.stringify({ answer: confirmation })
			})
		...
		if (asked >= MAX_Q) {
			guesserMessages.push({
				role: "user",
				content: "That was question 20. Output a final guess now."
			})
			const final = await callGuesser(guesserMessages)

			answererMessages.push({
				role: "user",
				content: `Is the answer "${final.text}"?`
			})
			const confirmation = await callAnswerer(answererMessages, answSys)
			const correct = confirmation === "Yes"
